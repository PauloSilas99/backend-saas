...
async function main() {
  ...
  const gestorEmail = process.env.GESTOR_EMAIL ?? 'gestor@saas.local';
  const gestorPassword = process.env.GESTOR_PASSWORD ?? 'Gestor@123456';
  ...
  const gestor = await prisma.user.upsert({
    ...
    update: { passwordHash: gestorHash, name: 'Gestor Demo', isActive: true },
    create: {
      email: gestorEmail,
